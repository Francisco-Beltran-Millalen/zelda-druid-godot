...
| **G5** | **Boundary validation communicates intent** — public surfaces validate inputs and name what was expected vs what was received. Bare `assert(condition)` is not enough. | Public surface methods missing assertions OR with assertions lacking an intent message. Best-effort heuristic. |
...
| **P5** | **No implicit ticks** — every script that overrides `_process` or `_physics_process` calls `set_*_process(false)` in `_ready()` unless it is the registered loop owner. | Process override without `set_*_process(false)` in `_ready()`. |
